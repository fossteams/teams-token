...
function getLoginURL(type: TeamsSkype) : string {
  ...
  switch (type) {
    case 'teams':
      ...
      loginUrl.searchParams.append('state', uuidv4());
      break;
    case 'skype':
      loginUrl.searchParams.append('response_type', 'token');
      loginUrl.searchParams.append('state', `${uuidv4()}|${SKYPE_RESOURCE}`);
  ...
  win.webContents.on('did-navigate', (e, url) => {
    if (url.startsWith('https://teams.microsoft.com/go')) {
      ...
      if (decoded.aud === TEAMS_APP_ID) {
        ...
        // Go To SKYPE_RESOURCE authorization
        authorize('skype');
